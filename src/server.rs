    ...
    C: Has<XSpanIdString> + Send + Sync,
{
    ...
    async fn get_table_by_id(
        ...
        match table {
            Ok(table) => Ok(GetTableByIdResponse::DetailedTableDescription(table)),
            Err(MetastoreError::TableAccessError(error)) => Ok(GetTableByIdResponse::GenericError(
                models::Error::from(error),
            )),
    ...
    async fn delete_table(&self, table_id: String, _: &C) -> Result<DeleteTableResponse, ApiError> {
        match self.metastore.write().await.delete_table(table_id) {
            Ok(_) => Ok(DeleteTableResponse::TableHasBeenDeletedSuccessfully),
            Err(MetastoreError::TableDeletionError(error)) => Ok(
                DeleteTableResponse::GenericError(models::Error::from(error)),
            ),
            _ => Err(ApiError("Internal server error".to_string())),
        }
    }
    ...
    async fn create_table(
        ...
        match self.metastore.write().await.create_table(table_schema) {
            Ok(id) => Ok(CreateTableResponse::TableCreatedSuccessfully(id)),
    ...
    async fn get_query_by_id(
        ...
        match self.metastore.read().await.get_query(query_id) {
            Ok(query) => Ok(GetQueryByIdResponse::DetailedQueryDescription(query)),
            Err(MetastoreError::QueryAccessError(error)) => Ok(GetQueryByIdResponse::GenericError(
                models::Error::from(error),
            )),
            _ => Err(ApiError("Internal server error".to_string())),
        }
    ...
    async fn get_query_result(
        ...
        match result {
            Ok(res) => Ok(GetQueryResultResponse::ResultOfSelectedQuery(res)),
            Err(MetastoreError::QueryAccessError(error)) => Ok(
                GetQueryResultResponse::GenericError(models::Error::from(error)),
            ),
            Err(MetastoreError::QueryResultAccessError(error)) => Ok(
                GetQueryResultResponse::GenericError_2(models::Error::from(error)),
            ),
            _ => Err(ApiError("Internal server error".to_string())),
        }
    }
    ...
    async fn get_query_error(
        ...
        match self.metastore.read().await.get_query_error(query_id) {
            ...
            Err(MetastoreError::QueryAccessError(error)) => Ok(
                GetQueryErrorResponse::GenericError(models::Error::from(error)),
            ),
            Err(MetastoreError::QueryErrorAccessError(error)) => Ok(
                GetQueryErrorResponse::GenericError_2(models::Error::from(error)),
            ),
            _ => Err(ApiError("Internal server error".to_string())),
        }
    }
